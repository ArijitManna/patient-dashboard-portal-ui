...
const AddDependantModal = ({ isOpen, onClose, onAddDependant, patientData, editDependant, isEdit }) => {
  // Array to store multiple dependents
  const [dependentsList, setDependentsList] = useState(() => {
    // If editing, initialize with the selected dependant
    if (isEdit && editDependant) {
      return [{
        id: editDependant.dependent_ID || editDependant.id,
        first_Name: editDependant.first_Name || '',
        middle_Name: editDependant.middle_Name || '',
        last_Name: editDependant.last_Name || '',
        age: editDependant.age || '',
        relationshipID: editDependant.relationshipID || '',
        profilePhoto: editDependant.profilePhoto || null,
      }];
    }
    // Default: empty add form
    return [{
      id: 1,
      first_Name: '',
      middle_Name: '',
  ...
  // Update dependentsList when editing
  React.useEffect(() => {
    if (isEdit && editDependant) {
      setDependentsList([{
        id: editDependant.dependent_ID || editDependant.id,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.message) {
      ...
      } else if (response.data && response.data.status && response.data.status === 'success' && response.data.msg) {
        Swal.fire({
          icon: 'success',
          title: 'Success',
        ...
        Swal.fire({
          icon: 'success',
          title: 'Success',
          text: `${dependentsList.length} dependant(s) added successfully!`,
        });
      }
    } catch (error) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Failed',
        text: error?.response?.data?.message || error?.message || 'Failed to add dependants. Please try again.',
      });
    }
  };
    ...
    <div className="modal-overlay">
      <div className="modal-container">
        <div className="modal-header">
          <h2 className="modal-title">{isEdit ? 'Edit Dependant' : 'Add Dependant'}</h2>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="form-actions">
            ...
            <button type="submit" className="submit-btn">
              {isEdit ? 'Update' : `Add ${dependentsList.length} Dependent${dependentsList.length > 1 ? 's' : ''}`}
            </button>
          </div>
        </form>
      </div>
